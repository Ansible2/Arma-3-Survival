while {true} do {
    ...
    // Keep players in the bulwark zone
    {
        switch (true) do {
            ...
            case ((_x distance2D bulwarkCity) > BLWK_playAreaRadius * 2): {
                _newLoc = [bulwarkBox] call bulwark_fnc_findPlaceAround;
                _x setPosASL _newLoc;
            };

            // If player is trying to leave, bounce them back.
            case ((_x distance2D bulwarkCity) > BLWK_playAreaRadius * 1.1): {
                _dir = bulwarkCity getDir _x;
                _newLoc = bulwarkCity getPos [(BLWK_playAreaRadius * 1.1)-8, _dir];
                _x setPosASL _newLoc;
