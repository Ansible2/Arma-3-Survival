class There_is_Only_War_Adeptus_Astartes_Combined
{
	displayName = "There is Only War - Adeptus Astartes (Combined)",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_BT",
		"TIOW_SM_Predator_BT",
		"TIOW_SM_Vindicator_BA",
		"TIOW_SM_Predator_BA",
		"TIOW_SM_Vindicator_BR",
		"TIOW_SM_Predator_BR",
		"TIOW_SM_Vindicator_DA",
		"TIOW_SM_Predator_DA",
		"TIOW_SM_Vindicator_IF",
		"TIOW_SM_Predator_IF",
		"TIOW_SM_Vindicator_IH",
		"TIOW_SM_Predator_IH",
		"TIOW_SM_Vindicator_RG",
		"TIOW_SM_Predator_SL",
		"TIOW_SM_Vindicator_SL",
		"TIOW_SM_Predator_SW",
		"TIOW_SM_Vindicator_SW",
		"TIOW_SM_Predator_UM",
		"TIOW_SM_Vindicator_UM",
		"TIOW_SM_Predator_WS",
		"TIOW_SM_Vindicator_WS"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_BT_4",
		"TIOW_Meltagunner_BT_3",
		"TIOW_Plasmagunner_BT_5",
		"TIOW_Sergeant_BT_1",
		"TIOW_Tactical_BT_7",
		"TIOW_Tactical_BT_6",
		"TIOW_Tactical_BT_5",
		"TIOW_Tactical_BT_4",
		"TIOW_Tactical_BT_3",
		"TIOW_Tactical_BT_1_Imperialis",
		"TIOW_Tactical_BT_1",
		"TIOW_Tactical_BT_2",
		"TIOW_HeavyBolter_BA_4",
		"TIOW_Meltagunner_BA_3",
		"TIOW_Plasmagunner_BA_5",
		"TIOW_Sergeant_BA_1",
		"TIOW_Tactical_BA_7",
		"TIOW_Tactical_BA_6",
		"TIOW_Tactical_BA_5",
		"TIOW_Tactical_BA_4",
		"TIOW_Tactical_BA_3",
		"TIOW_Tactical_BA_1_Imperialis",
		"TIOW_Tactical_BA_1",
		"TIOW_Tactical_BA_2",
		"TIOW_HeavyBolter_BR_4",
		"TIOW_Meltagunner_BR_3",
		"TIOW_Plasmagunner_BR_5",
		"TIOW_Sergeant_BR_1",
		"TIOW_Tactical_BR_7",
		"TIOW_Tactical_BR_6",
		"TIOW_Tactical_BR_5",
		"TIOW_Tactical_BR_4",
		"TIOW_Tactical_BR_3",
		"TIOW_Tactical_BR_1_Imperialis",
		"TIOW_Tactical_BR_1",
		"TIOW_Tactical_BR_2",
		"TIOW_HeavyBolter_DA_HH_4",
		"TIOW_Meltagunner_DA_HH_3",
		"TIOW_Plasmagunner_DA_HH_5",
		"TIOW_Tactical_DA_HH_7",
		"TIOW_Tactical_DA_HH_6",
		"TIOW_Tactical_DA_HH_5",
		"TIOW_Tactical_DA_HH_4",
		"TIOW_Tactical_DA_HH_3",
		"TIOW_Tactical_DA_HH_1_Imperialis",
		"TIOW_Tactical_DA_HH_1",
		"TIOW_Tactical_DA_HH_2",
		"TIOW_HeavyBolter_DA_4",
		"TIOW_Meltagunner_DA_3",
		"TIOW_Plasmagunner_DA_5",
		"TIOW_Sergeant_DA_1",
		"TIOW_Tactical_DA_7",
		"TIOW_Tactical_DA_6",
		"TIOW_Tactical_DA_5",
		"TIOW_Tactical_DA_4",
		"TIOW_Tactical_DA_3",
		"TIOW_Tactical_DA_1_Imperialis",
		"TIOW_Tactical_DA_1",
		"TIOW_Tactical_DA_2",
		"TIOW_HeavyBolter_IF_4",
		"TIOW_Meltagunner_IF_3",
		"TIOW_Plasmagunner_IF_5",
		"TIOW_Sergeant_IF_1",
		"TIOW_Tactical_IF_7",
		"TIOW_Tactical_IF_6",
		"TIOW_Tactical_IF_5",
		"TIOW_Tactical_IF_4",
		"TIOW_Tactical_IF_3",
		"TIOW_Tactical_IF_1_Imperialis",
		"TIOW_Tactical_IF_1",
		"TIOW_Tactical_IF_2",
		"TIOW_HeavyBolter_IH_4",
		"TIOW_Meltagunner_IH_3",
		"TIOW_Plasmagunner_IH_5",
		"TIOW_Sergeant_IH_1",
		"TIOW_Tactical_IH_7",
		"TIOW_Tactical_IH_6",
		"TIOW_Tactical_IH_5",
		"TIOW_Tactical_IH_4",
		"TIOW_Tactical_IH_3",
		"TIOW_Tactical_IH_1_Imperialis",
		"TIOW_Tactical_IH_1",
		"TIOW_Tactical_IH_2",
		"TIOW_HeavyBolter_RG_4",
		"TIOW_Meltagunner_RG_3",
		"TIOW_Plasmagunner_RG_5",
		"TIOW_Sergeant_RG_1",
		"TIOW_Tactical_RG_7",
		"TIOW_Tactical_RG_6",
		"TIOW_Tactical_RG_5",
		"TIOW_Tactical_RG_4",
		"TIOW_Tactical_RG_3",
		"TIOW_Tactical_RG_1_Imperialis",
		"TIOW_Tactical_RG_1",
		"TIOW_Tactical_RG_2",
		"TIOW_HeavyBolter_SL_4",
		"TIOW_Meltagunner_SL_3",
		"TIOW_Plasmagunner_SL_5",
		"TIOW_Sergeant_SL_1",
		"TIOW_Tactical_SL_7",
		"TIOW_Tactical_SL_6",
		"TIOW_Tactical_SL_5",
		"TIOW_Tactical_SL_4",
		"TIOW_Tactical_SL_3",
		"TIOW_Tactical_SL_1_Imperialis",
		"TIOW_Tactical_SL_1",
		"TIOW_Tactical_SL_2",
		"TIOW_HeavyBolter_SW_4",
		"TIOW_Meltagunner_SW_3",
		"TIOW_Plasmagunner_SW_5",
		"TIOW_Sergeant_SW_1",
		"TIOW_Tactical_SW_7",
		"TIOW_Tactical_SW_6",
		"TIOW_Tactical_SW_5",
		"TIOW_Tactical_SW_4",
		"TIOW_Tactical_SW_3",
		"TIOW_Tactical_SW_1_Imperialis",
		"TIOW_Tactical_SW_1",
		"TIOW_Tactical_SW_2",
		"TIOW_HeavyBolter_UM_4",
		"TIOW_Meltagunner_UM_3",
		"TIOW_Plasmagunner_UM_5",
		"TIOW_Sergeant_UM_1",
		"TIOW_Tactical_UM_7",
		"TIOW_Tactical_UM_6",
		"TIOW_Tactical_UM_5",
		"TIOW_Tactical_UM_4",
		"TIOW_Tactical_UM_3",
		"TIOW_Tactical_UM_1_Imperialis",
		"TIOW_Tactical_UM_1",
		"TIOW_Tactical_UM_2",
		"TIOW_HeavyBolter_WS_4",
		"TIOW_Meltagunner_WS_3",
		"TIOW_Plasmagunner_WS_5",
		"TIOW_Sergeant_WS_1",
		"TIOW_Tactical_WS_7",
		"TIOW_Tactical_WS_6",
		"TIOW_Tactical_WS_5",
		"TIOW_Tactical_WS_4",
		"TIOW_Tactical_WS_3",
		"TIOW_Tactical_WS_1_Imperialis",
		"TIOW_Tactical_WS_1",
		"TIOW_Tactical_WS_2"

	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_BT",
		"TIOW_SM_Rhino_BA",
		"TIOW_SM_Rhino_BR",
		"TIOW_SM_Rhino_DA",
		"TIOW_SM_Rhino_IF",
		"TIOW_SM_Rhino_IH",
		"TIOW_SM_Rhino_RG",
		"TIOW_SM_Rhino_SL",
		"TIOW_SM_Rhino_SW",
		"TIOW_SM_Rhino_UM",
		"TIOW_SM_Rhino_WS"
	};
};

class There_is_Only_War_Adeptus_Astartes_BT
{
	displayName = "There is Only War - Adeptus Astartes: Black Templars",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_BT",
		"TIOW_SM_Predator_BT"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_BT_4",
		"TIOW_Meltagunner_BT_3",
		"TIOW_Plasmagunner_BT_5",
		"TIOW_Sergeant_BT_1",
		"TIOW_Tactical_BT_7",
		"TIOW_Tactical_BT_6",
		"TIOW_Tactical_BT_5",
		"TIOW_Tactical_BT_4",
		"TIOW_Tactical_BT_3",
		"TIOW_Tactical_BT_1_Imperialis",
		"TIOW_Tactical_BT_1",
		"TIOW_Tactical_BT_2"

	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_BT"
	};
};

class There_is_Only_War_Adeptus_Astartes_Combined_BA
{
	displayName = "There is Only War - Adeptus Astartes: Blood Angels",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_BA",
		"TIOW_SM_Predator_BA"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_BA_4",
		"TIOW_Meltagunner_BA_3",
		"TIOW_Plasmagunner_BA_5",
		"TIOW_Sergeant_BA_1",
		"TIOW_Tactical_BA_7",
		"TIOW_Tactical_BA_6",
		"TIOW_Tactical_BA_5",
		"TIOW_Tactical_BA_4",
		"TIOW_Tactical_BA_3",
		"TIOW_Tactical_BA_1_Imperialis",
		"TIOW_Tactical_BA_1",
		"TIOW_Tactical_BA_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_BA"
	};
};

class There_is_Only_War_Adeptus_Astartes_BR
{
	displayName = "There is Only War - Adeptus Astartes: Blood Ravens",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_BR",
		"TIOW_SM_Predator_BR"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_BR_4",
		"TIOW_Meltagunner_BR_3",
		"TIOW_Plasmagunner_BR_5",
		"TIOW_Sergeant_BR_1",
		"TIOW_Tactical_BR_7",
		"TIOW_Tactical_BR_6",
		"TIOW_Tactical_BR_5",
		"TIOW_Tactical_BR_4",
		"TIOW_Tactical_BR_3",
		"TIOW_Tactical_BR_1_Imperialis",
		"TIOW_Tactical_BR_1",
		"TIOW_Tactical_BR_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_BR"
	};
};

class There_is_Only_War_Adeptus_Astartes_DA
{
	displayName = "There is Only War - Adeptus Astartes: Dark Angels",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_DA",
		"TIOW_SM_Predator_DA"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_DA_HH_4",
		"TIOW_Meltagunner_DA_HH_3",
		"TIOW_Plasmagunner_DA_HH_5",
		"TIOW_Tactical_DA_HH_7",
		"TIOW_Tactical_DA_HH_6",
		"TIOW_Tactical_DA_HH_5",
		"TIOW_Tactical_DA_HH_4",
		"TIOW_Tactical_DA_HH_3",
		"TIOW_Tactical_DA_HH_1_Imperialis",
		"TIOW_Tactical_DA_HH_1",
		"TIOW_Tactical_DA_HH_2",
		"TIOW_HeavyBolter_DA_4",
		"TIOW_Meltagunner_DA_3",
		"TIOW_Plasmagunner_DA_5",
		"TIOW_Sergeant_DA_1",
		"TIOW_Tactical_DA_7",
		"TIOW_Tactical_DA_6",
		"TIOW_Tactical_DA_5",
		"TIOW_Tactical_DA_4",
		"TIOW_Tactical_DA_3",
		"TIOW_Tactical_DA_1_Imperialis",
		"TIOW_Tactical_DA_1",
		"TIOW_Tactical_DA_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_DA"
	};
};

class There_is_Only_War_Adeptus_Astartes_IF
{
	displayName = "There is Only War - Adeptus Astartes: Imperial Fists",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_IF",
		"TIOW_SM_Predator_IF"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_IF_4",
		"TIOW_Meltagunner_IF_3",
		"TIOW_Plasmagunner_IF_5",
		"TIOW_Sergeant_IF_1",
		"TIOW_Tactical_IF_7",
		"TIOW_Tactical_IF_6",
		"TIOW_Tactical_IF_5",
		"TIOW_Tactical_IF_4",
		"TIOW_Tactical_IF_3",
		"TIOW_Tactical_IF_1_Imperialis",
		"TIOW_Tactical_IF_1",
		"TIOW_Tactical_IF_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_IF"
	};
};

class There_is_Only_War_Adeptus_Astartes_IH
{
	displayName = "There is Only War - Adeptus Astartes: Iron Hands",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_IH",
		"TIOW_SM_Predator_IH"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_IH_4",
		"TIOW_Meltagunner_IH_3",
		"TIOW_Plasmagunner_IH_5",
		"TIOW_Sergeant_IH_1",
		"TIOW_Tactical_IH_7",
		"TIOW_Tactical_IH_6",
		"TIOW_Tactical_IH_5",
		"TIOW_Tactical_IH_4",
		"TIOW_Tactical_IH_3",
		"TIOW_Tactical_IH_1_Imperialis",
		"TIOW_Tactical_IH_1",
		"TIOW_Tactical_IH_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_IH"
	};
};

class There_is_Only_War_Adeptus_Astartes_RG
{
	displayName = "There is Only War - Adeptus Astartes: Raven Guard",

	heavyArmor[] = {
		"TIOW_SM_Vindicator_RG"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_RG_4",
		"TIOW_Meltagunner_RG_3",
		"TIOW_Plasmagunner_RG_5",
		"TIOW_Sergeant_RG_1",
		"TIOW_Tactical_RG_7",
		"TIOW_Tactical_RG_6",
		"TIOW_Tactical_RG_5",
		"TIOW_Tactical_RG_4",
		"TIOW_Tactical_RG_3",
		"TIOW_Tactical_RG_1_Imperialis",
		"TIOW_Tactical_RG_1",
		"TIOW_Tactical_RG_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_RG"
	};
};

class There_is_Only_War_Adeptus_Astartes_SL
{
	displayName = "There is Only War - Adeptus Astartes: Salamanders",

	heavyArmor[] = {
		"TIOW_SM_Predator_SL",
		"TIOW_SM_Vindicator_SL"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_SL_4",
		"TIOW_Meltagunner_SL_3",
		"TIOW_Plasmagunner_SL_5",
		"TIOW_Sergeant_SL_1",
		"TIOW_Tactical_SL_7",
		"TIOW_Tactical_SL_6",
		"TIOW_Tactical_SL_5",
		"TIOW_Tactical_SL_4",
		"TIOW_Tactical_SL_3",
		"TIOW_Tactical_SL_1_Imperialis",
		"TIOW_Tactical_SL_1",
		"TIOW_Tactical_SL_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_SL"
	};
};

class There_is_Only_War_Adeptus_Astartes_SW
{
	displayName = "There is Only War - Adeptus Astartes: Space Wolves",

	heavyArmor[] = {
		"TIOW_SM_Predator_SW",
		"TIOW_SM_Vindicator_SW"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_SW_4",
		"TIOW_Meltagunner_SW_3",
		"TIOW_Plasmagunner_SW_5",
		"TIOW_Sergeant_SW_1",
		"TIOW_Tactical_SW_7",
		"TIOW_Tactical_SW_6",
		"TIOW_Tactical_SW_5",
		"TIOW_Tactical_SW_4",
		"TIOW_Tactical_SW_3",
		"TIOW_Tactical_SW_1_Imperialis",
		"TIOW_Tactical_SW_1",
		"TIOW_Tactical_SW_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_SW"
	};
};

class There_is_Only_War_Adeptus_Astartes_UM
{
	displayName = "There is Only War - Adeptus Astartes: Ultramarines",

	heavyArmor[] = {
		"TIOW_SM_Predator_UM",
		"TIOW_SM_Vindicator_UM"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_UM_4",
		"TIOW_Meltagunner_UM_3",
		"TIOW_Plasmagunner_UM_5",
		"TIOW_Sergeant_UM_1",
		"TIOW_Tactical_UM_7",
		"TIOW_Tactical_UM_6",
		"TIOW_Tactical_UM_5",
		"TIOW_Tactical_UM_4",
		"TIOW_Tactical_UM_3",
		"TIOW_Tactical_UM_1_Imperialis",
		"TIOW_Tactical_UM_1",
		"TIOW_Tactical_UM_2"
	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_UM"
	};
};

class There_is_Only_War_Adeptus_Astartes_WS
{
	displayName = "There is Only War - Adeptus Astartes: White Scars",

	heavyArmor[] = {
		"TIOW_SM_Predator_WS",
		"TIOW_SM_Vindicator_WS"
	};
	
	infantry[] = {
		"TIOW_HeavyBolter_WS_4",
		"TIOW_Meltagunner_WS_3",
		"TIOW_Plasmagunner_WS_5",
		"TIOW_Sergeant_WS_1",
		"TIOW_Tactical_WS_7",
		"TIOW_Tactical_WS_6",
		"TIOW_Tactical_WS_5",
		"TIOW_Tactical_WS_4",
		"TIOW_Tactical_WS_3",
		"TIOW_Tactical_WS_1_Imperialis",
		"TIOW_Tactical_WS_1",
		"TIOW_Tactical_WS_2"

	};
	
	lightArmor[] = {
		"TIOW_SM_Rhino_WS"
	};
};